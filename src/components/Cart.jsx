  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, overflow: 'hidden', pointerEvents: open ? 'auto' : 'none' }} aria-hidden={!open}>
      ...
      <aside ref={panelRef} role="dialog" aria-modal="true" aria-label={C.title} style={sheet
        ...
        ? { position: 'absolute', left: 0, right: 0, bottom: 0, maxHeight: '86%', background: 'var(--pum-cream)', borderRadius: '26px 26px 0 0', boxShadow: '0 -18px 44px rgba(13,30,58,.3)', transform: open ? 'translateY(0)' : 'translateY(105%)', transition: 'transform .32s cubic-bezier(.4,1.1,.5,1)', display: 'flex', flexDirection: 'column' }
